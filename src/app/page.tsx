...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#FCFAF6] text-[#3B342B] pb-24 px-4 font-sans">
      <div className="max-w-md mx-auto pt-6">
        
        {/* 상단 헤더: 황금 벼색과 로열 블루의 조화 */}
        <header className="flex items-center justify-between mb-4 px-1">
          <div>
            <div className="flex items-center gap-1.5 text-blue-600">
              <Sparkles className="w-4 h-4 text-amber-500" />
              <span className="text-xs font-black tracking-widest uppercase text-blue-600">Chiu's Daily Record</span>
            </div>
            <h1 className="text-2xl font-black text-amber-950 tracking-tight flex items-center gap-1">
              치우의 하루하루 <span className="text-xl">🌾</span>
            </h1>
          </div>
          
          <button
            onClick={() => setIsFormOpen(!isFormOpen)}
            className="flex items-center gap-1.5 text-xs font-bold px-3.5 py-2 rounded-full bg-blue-600 text-white shadow-sm hover:bg-blue-700 active:scale-95 transition-all"
          >
            {isFormOpen ? (
              <>접기 <ChevronUp className="w-3.5 h-3.5" /></>
            ) : (
              <>기록 쓰기 <Plus className="w-3.5 h-3.5" /></>
            )}
          </button>
        </header>

        {/* 뷰 전환 탭: 피드 vs 달력/통계 */}
        <nav className="flex bg-amber-100/60 p-1 rounded-2xl mb-5">
          <button
          ...
          >
            <List className="w-3.5 h-3.5" /> 카드 피드
          </button>
          <button
          ...
          >
            <BarChart3 className="w-3.5 h-3.5" /> 달력 & 월간 통계
          </button>
        </nav>

        {/* 기록 입력 폼 */}
        {isFormOpen && (
          <section className="bg-white rounded-3xl p-5 mb-6 border border-amber-200/80 shadow-sm transition-all animate-in fade-in slide-in-from-top-2 duration-200">
            <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 transition-all ${
                        item.val
                          ...
                          : 'bg-white border border-amber-200/70 text-amber-800/80'
                      }`}
                    >
              ...
              <div className="space-y-1.5">
                ...
                {previewUrl ? (
                  ...
                  <button
                    ...
                    className="w-full py-4 rounded-2xl border-2 border-dashed border-amber-200 bg-amber-50/30 flex flex-col items-center justify-center gap-1 text-amber-800 active:scale-[0.99]"
                  >
                    <Camera className="w-5 h-5 text-blue-600" />
                    <span className="text-xs font-bold">오늘 치우의 베스트 컷 넣기</span>
                  </button>
                )}
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-black text-sm flex items-center justify-center gap-2 active:scale-95 transition-all shadow-sm disabled:opacity-50"
              >
                {submitting ? (
          ...
          <section className="space-y-4">
            {loading ? (
              ...
              <div className="bg-white rounded-3xl p-10 text-center border border-amber-100 shadow-sm">
                ...
                <p className="text-xs text-amber-700/60 mt-1">상단 버튼을 눌러 첫 일기를 작성해 보세요!</p>
              </div>
            ) : (
              logs.map((log) => (
                <article
                  key={log.id}
                  className="bg-white rounded-3xl p-4 border border-amber-100 shadow-sm space-y-3"
                >
                  {/* 날짜 및 삭제 버튼 */}
        ...
        {currentView === 'calendar' && (
          <section className="space-y-4">
            ...
            {/* 달력 그리드 (사진 꽉 차게 + 산책 발바닥 도장) */}
            <div className="bg-white rounded-3xl p-3.5 border border-amber-100 shadow-sm">
              ...
              <div className="grid grid-cols-7 gap-1">
                {calendarDays.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={item.dateStr}
                      className="relative aspect-square rounded-xl overflow-hidden border border-stone-100 bg-stone-50/50 flex flex-col justify-between p-1 group"
                    >
                      {/* 그날의 사진이 있으면 배경으로 꽉 채움 */}
                      {log?.photo_url && (
                        <img
                          src={log.photo_url}
                          alt=""
